...
%TODO!: Фиксануть где то есть русские символы в оформлении математики!
...
где  $\frac{dw}{dt}$ называется угловым ускорением и обозначается $\varepsilon$
\[ w_\tau = R \cdot \varepsilon \text{ --- \textbf{\textit{касательное ускорение}}} \]
\[w_n = \frac{v^2}{R} = \omega^2 \cdot R \text{ --- \textbf{\textit{нормальное ускорение}}} \]
\[ \vec w = \vec w_\tau + \vec w_n\]
\[ w = R \sqrt{w_n^4 + \varepsilon^2} \]
...
\[ \vec w = \frac{dv}{dt} = \frac{d}{dt} [\vec \omega, \vec r] = [\frac{d\omega}{dt}, \vec r] + [\vec \omega, \frac{dr}{dt}] \] так как $\vec \omega$ - вектор, то и угловое ускорение $\vec \varepsilon = \frac{d\omega}{dt}$ - вектор.

\vspace{5px}

Получаем :
\[ \vec w = \frac{d}{dt} [\vec \omega, \vec r] = [\frac{d\omega}{dt}, \vec r] + [\vec \omega, \frac{dr}{dt}] = [\vec \varepsilon, \vec r] + [\vec \omega, \vec v]  = [\vec \varepsilon, \vec r] + [\omega, [\omega,r]] \text{ ,где $ [\vec \varepsilon, \vec r] = w_\tau $ , а  $[\omega, [\omega,r]] = w_n$}\] %TODO: Переполнение фиксануть
...
\[\vec w_M = \vec w_A + [\vec \varepsilon, \vec a] + [\vec w, [\vec w, \vec a]]\]
где $\vec w_A$ - поступательное ускорение, а $[\vec \varepsilon, \vec a] + [\vec w, [\vec w, \vec a]]$ - ускорение вращательного движения
