...
При разделении проводника одна половина окажется положительно заряженной(+), другая отрицательно(-)ю При этом линии
...
рисунок3
Рассмотрим рисунок, на острие будет очень высокая напряженность поля. Ионизация воздуха в результате отрыва
...
Из предыдущих формул, известна зависимость напряжение электрического поля и потенциала соотвественно. Рассмотрим
поверхность проводника, поскольку движение частиц прекращается напряженность такого электрического поля будет равна нулю:
...
Пределльное значение напряжения конденсатора $U_{max}$, если превысить это значение может случится так называемый пробой конденсаторов.
...
Возьмем два заряда, q1 q2,

Обозначим потенциал создаваемый первым зарядом в точке q2 назовем $\phi_2$, и наоборот потенциал вторым зарядом в точке q1 назовем $\phi_1$
...
К этой системе добавим заряд q3 и добавим обозначение $\phi_3$ - потенциал который создают заряды q1 q2 в точке q3,
тогда выразим энергию заряда q3 в этом поле:
...
\[W = W_{12} + W_{13} + W_{23} = \frac{1}{4 \pi \epsilon_0} (\frac{q_1 \cdot q_2}{r_{12}} + \frac{q_1 \cdot q_3}{r_{13}}+ \frac{q_2 \cdot q_3}{r_{23}}) =
    ...
    \[\frac{1}{2}(\frac{q_1}{4 \pi \epsilon_0}(\frac{q_2}{r_{12}} + \frac{q_3}{r_13}) + \frac{q_2}{4 \pi \epsilon_0}(\frac{q_1}{r_{12}} + \frac{q_3}{r_13}) + \frac{q_3}{4 \pi \epsilon_0}(\frac{q_1}{r_{12}} + \frac{q_2}{r_13}) )\]
    = \frac{1}{2} (q_1 \phi_1 + q_2 \phi_2  + q_3 \phi_3)
\]
Итого энергия системы зарядов равнаЖ
...
Наличие диэлектрика влияет на общую энергию электрического поля.Будем расматривать плоский конденсатор, подставим в эту формулы выражение для емкость
...
В анизотропных средах заметим что направления D и E не совпарают тогда берем скалярное поризведение вектором D и E:
...
За направление тока принимается движения положительно заряженных частиц.Такое движение принято характеризовать силой тока  --- заряд прошедший через сечение проводника за единицу времени.
\[i = \frac{dq}{dt} = \frac{dq ^{+}}{dt} + \frac{dq ^{-}}{dt}\] --- сила пременного электрического тока.

Поскольку наши частицы есть свообоюные частицы, то движутся они хаотично. тогда можем определить скорость хаотического движения зарядов $\vec v$,
а u - скрость упорядоченного движения заряда. Наши зар

Плотность тока -

Сила тока величина скалярная, плотность тока величина векторная - $\iota = \frac{I}{S}$, плотность тока всегда направлена по линии направление движенния тока.
...
В = $\frac{\text{Дж}}{\text{Кл}} = \frac{\text{кг м^2}}{A c^2}$
\end{verbatim}
\subsection{Электродвижущая сила}
Возьмем уединенный проводник и поместим его в электрическое поле. Так для того


Эти сторонние силы совершают работу по переносу заряда
Величина равная работе сторонних сил, деленная на велиичну перенесенного заряда называется электродвижующей силы.
